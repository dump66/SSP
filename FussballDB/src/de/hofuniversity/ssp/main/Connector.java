package de.hofuniversity.ssp.main;
...
public class Connector {
	...
	Connection c = DriverManager.getConnection("jdbc:mysql://localhost:3306/", "Student", "Hof");
	return c;
    }
	...
	for (Object o : elements) {
	    String sql = "insert into fussballdb.";
	    if (o instanceof Team) {
		Team t = (Team) o;
		sql = sql+"verein(i_id, v_name, v_logo, fk_stadion) values(" + t.getId() + ",'" + t.getName() + "','" + t.getIcon()
			+ "'," + t.getStadium().getId() + ");";
	    } else if (o instanceof Player) {
		Player p = (Player) o;
		sql = sql+"spieler(i_id, v_name, fk_verein) values(" + p.getId() + ",'" + p.getName() + "'," + p.getTeam().getId()
			+ ");";
	    } else if (o instanceof Stadium) {
		Stadium s = (Stadium) o;
		sql = sql+"stadion(i_id, v_name) values(" + s.getId() + ",'" + s.getName() + "');";
	    } else if (o instanceof Match) {
		Match m = (Match) o;
		sql = sql+"spiel(i_id, i_saison, v_datum, i_zuschauer, fk_stadion, v_endergebnis, v_hzergebnis, i_punkteHeim, i_punkteGast, fk_heimVerein, fk_gastVerein) "
			+ "values(" + m.getId() + "," + m.getSeason() + ",'" + m.getDate() + "'," + m.getVisitors() + ","
			+ m.getStadium().getId() + ",'" + m.getFinalScore() + "','" + m.getHtScore() + "'," + m.getPtsHome() + ","
			+ m.getPtsGuest() + "," + m.getHomeTeam().getId() + "," + m.getGuestTeam().getId() + ");";
	    } else if (o instanceof Goal) {
		Goal g = (Goal) o;
		sql = sql+"tor(i_id, i_torHeim, i_torGast, i_minute, fk_spieler, fk_spiel) " + "values(" + g.getId() + ","
			+ g.getGoalHome() + "," + g.getGoalGuest() + "," + g.getMinute() + "," + g.getPlayer().getId() + ","
			+ g.getMatch().getId() + ");";
	    } else {
	...
	con.executeStatement(con.getDDL("/createDB.sql"));
	con.executeStatement(con.getDDL("/import.sql"));
